...
class SpottedLanternFlyEggHatchLayer extends BasicLayer{
	
    public function __construct($long_forecast=false){
        $title = "Spotted Lantern Fly Egg Hatch Forecast";
        ...
        $output_path = "slf_egg_hatch.png";        
        $curl_url = "http://geoserver.usanpn.org/geoserver/wms?service=WMS&request=GetMap&bbox=-14000000,2700000,-7000000,6450000&srs=EPSG:3857&layers=gdd:slf_egg_hatch&width=1500&height=800&ENV=doy:".(date('z')+1)."&time="  . date_format(new DateTime(), "Y-m-d") . "&format=image/png&transparent=true";
        ...
        $this->subtitle = "Threshold: 364 GDD(F), Base: 50F, Start: Jan 1";
        $this->legend_path = $this->base_legend_path . "legend-slf-adult" . $this->extension;
        $this->overlay_path = $this->base_overlay_path . "overlay-slf-adult.png";   
